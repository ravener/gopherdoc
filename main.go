...
func main() {
	...
	log.SetPrefix("doc: ")
	dirsInit()
	err := do(colorable.NewColorableStdout(), flag.CommandLine, os.Args[1:])
	if err != nil {
		log.Fatal(err)
	}
}
...
func do(writer io.Writer, flagSet *flag.FlagSet, args []string) (err error) {
	...
	flagSet.StringVar(&style, "style", "", "change chroma's style (can also set GDOC_STYLE environment variable, default is 'vim')")
	flagSet.Parse(args)
	var paths []string
